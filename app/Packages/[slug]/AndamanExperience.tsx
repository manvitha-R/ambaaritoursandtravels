...
// 0 Andaman.png poster (cover tile), 1 Cellular Jail, 2 Radhanagar Beach,
// 3 Havelock Island, 4 Bharatpur Beach, 5 Laxmanpur Beach. Day 5 (departure)
// has no dedicated photo, so it falls back to the poster.
const DAY_IMAGE_INDEX: Record<number, number> = { 1: 1, 2: 2, 3: 3, 4: 4, 5: 0 };

export default function AndamanExperience({
  pkg,
  itinerary,
  highlights,
  bookNowHref,
  ...
  const images: string[] = pkg.images?.length ? pkg.images : ["/Images/Andaman.png"];
        ...
        {mosaicImages.length <= 1 && (
          <div className="mb-5">
            ...
            <h1 className="text-lg md:text-2xl font-semibold text-white">
              Andaman Islands Tour <span className="text-amber-200 font-normal text-sm md:text-base">· 4 Nights / 5 Days</span>
            </h1>
          </div>
        )}
        {mosaicImages.length > 1 && (
