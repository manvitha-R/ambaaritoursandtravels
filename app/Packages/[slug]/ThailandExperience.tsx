...
// Curated so each itinerary day pairs with the photo that actually depicts it,
// assuming photos are uploaded in this order via the admin dashboard:
// 0 = Pattaya sign (cover/establishing shot), 1 = Sriracha Tiger Zoo,
// 2 = Pattaya Floating Market, 3 = Big Buddha Temple, 4 = Safari World.
const DAY_IMAGE_INDEX: Record<number, number> = { 0: 0, 1: 1, 2: 2, 3: 3, 4: 0, 5: 4 };

export default function ThailandExperience({
  pkg,
  itinerary,
  highlights,
  bookNowHref,
  ...
  const images: string[] = pkg.images?.length ? pkg.images : ["/Images/Thailand.png"];
        ...
        {mosaicImages.length <= 1 && (
          <div className="mb-5">
            ...
            <h1 className="text-lg md:text-2xl font-semibold text-white">
              Thailand <span className="text-amber-200 font-normal text-sm md:text-base">· 4 Nights / 5 Days</span>
            </h1>
          </div>
        )}
        {mosaicImages.length > 1 && (
